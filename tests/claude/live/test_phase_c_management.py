...
class TestMemAdd:
    ...
    def test_knowledge_add_embedding_persisted(self, http_client):
        ...
        # Count before
        before_raw = docker_psql("SELECT COUNT(*) FROM mem0_memories").strip()
        before_match = re.search(r"(\d+)", before_raw)
        before = int(before_match.group(1)) if before_match else 0

        # Add a memory with clear extractable facts
        ...
        time.sleep(3)
        after_raw = docker_psql("SELECT COUNT(*) FROM mem0_memories").strip()
        after_match = re.search(r"(\d+)", after_raw)
        after = int(after_match.group(1)) if after_match else 0

        assert after > before, (
            f"mem0_memories row count did not increase: before={before}, after={after}"
        )
