...
def mock_pg_pool():
    pool = AsyncMock()
    with patch("app.database._pg_pool", pool), patch(
        "app.database.get_pg_pool", return_value=pool
    ):
        yield pool


@pytest.fixture
def mock_pg_for_locks():
    pool = AsyncMock()
    with patch("context_broker_ae.memory_extraction.get_pg_pool", return_value=pool):
        yield pool
...
class TestAcquireExtractionLock:
    @pytest.mark.asyncio
    async def test_lock_acquired(self, mock_pg_for_locks):
        from context_broker_ae.memory_extraction import acquire_extraction_lock

        mock_pg_for_locks.fetchval.return_value = True
    ...
    @pytest.mark.asyncio
    async def test_lock_not_acquired(self, mock_pg_for_locks):
        from context_broker_ae.memory_extraction import acquire_extraction_lock

        mock_pg_for_locks.fetchval.return_value = False
        state = {"conversation_id": str(uuid.uuid4()), "config": {"tuning": {}}}
        result = await acquire_extraction_lock(state)
...
class TestBuildExtractionText:
    ...
    async def test_skips_null_content(self):
        msgs = [
            self._make_msg(content="This is a valid conversation message"),
            self._make_msg(content=None),
            self._make_msg(content=""),
            self._make_msg(content="This is also a valid message"),
        ]
        state = {
            "messages": msgs,
            "config": {"tuning": {}},
        ...
        result = await build_extraction_text(state)
        assert "valid conversation message" in result["extraction_text"]
        assert "also a valid message" in result["extraction_text"]
    ...
    async def test_respects_max_chars(self):
        ...
        state = {
            "messages": msgs,
            "config": {"tuning": {"extraction_max_chars": 10000}},
            "selected_message_ids": [],
            "fully_extracted_ids": [],
            "extraction_text": "",
            "error": None,
    ...
    async def test_redacts_secrets_in_output(self):
        msgs = [
            self._make_msg(content='api_key="sk-proj-abcdefghijklmnopqrstuvwxyz123456"')
        ]
        state = {
            "messages": msgs,
            "config": {"tuning": {}},
...
class TestRunMem0Extraction:
    ...
    async def test_mem0_add_called_with_correct_args(self):
        ...
        state = {
            ...
            "config": {},
            "extraction_text": "The sky is blue.",
            "user_id": "user123",
            "selected_message_ids": ["id1"],
            "extracted_count": 0,
            "error": None,
    ...
    async def test_mem0_exception_triggers_reset(self):
        ...
        with patch(
            ...
            "context_broker_ae.memory.mem0_client.reset_mem0_client"
        ) as mock_reset:
            result = await run_mem0_extraction(state)
...
class TestReleaseExtractionLock:
    @pytest.mark.asyncio
    async def test_releases_when_acquired(self, mock_pg_for_locks):
        ...
        assert result == {}
        mock_pg_for_locks.execute.assert_called_once()

    @pytest.mark.asyncio
    async def test_skips_when_not_acquired(self, mock_pg_for_locks):
        from context_broker_ae.memory_extraction import release_extraction_lock

        state = {"lock_key": "12345", "lock_token": None, "lock_acquired": False}
        await release_extraction_lock(state)
        mock_pg_for_locks.execute.assert_not_called()

    @pytest.mark.asyncio
    async def test_skips_when_no_lock_key(self, mock_pg_for_locks):
        from context_broker_ae.memory_extraction import release_extraction_lock

        state = {"lock_key": "", "lock_token": None, "lock_acquired": True}
        await release_extraction_lock(state)
        mock_pg_for_locks.execute.assert_not_called()
...
class TestRouting:
    ...
    def test_route_after_build_text_error(self):
        assert (
            route_after_build_text({"error": "some error"}) == "release_extraction_lock"
        )
