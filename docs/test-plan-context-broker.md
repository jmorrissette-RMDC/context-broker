...
- **Test Case:** Verify all MCP tools.
  - **Input:** For each tool listed in REQ §4.6 (e.g., `conv_create_conversation`, `knowledge_search`, `metrics_get`), execute calls with: (a) valid parameters, (b) missing required parameters, and (c) parameters with invalid data types.
...
### 4.30 Context Stress Testing (Compaction Pipeline)

Follows the general approach defined in `docs/guides/context-stress-testing-guide.md`, applied to the Context Broker's compaction pipeline.

**CB-Specific Configuration:**
- **Ingestion endpoint:** MCP tool `conv_store_message`
- **Context retrieval:** MCP tool `get_context` with `build_type` and `budget`
- **Metrics collector:** `tests/stress/metrics_collector.py` — queries `conversation_summaries` table for tier distribution, compaction cycle count, token counts
- **Quality judge:** `tests/stress/quality_judge.py` — LLM-as-judge evaluation of assembled context quality
- **Config:** `tests/stress/config.yml` — target host, budget tiers, build type, Gemini talker model

**Test Cases:**

- **Test Case:** Progressive compaction at each budget tier (8K → 1M).
  - **Input:** Prime conversation with session file data to ~90% of trigger, then live conversation to push over.
  - **Expected Outcome:** Compaction triggers at each tier. Tier distribution matches deadband expectations: tier 1 (live) 20-73%, tier 2 (chunks) 6-12%, tier 3 (archival) 2%.

- **Test Case:** Cold recall after compaction.
  - **Input:** After compaction triggers, ask the Imperator about content from the earliest topic in the conversation.
  - **Expected Outcome:** Agent recalls key facts from earlier topics. Hallucination or inability to answer indicates compaction is too lossy.

- **Test Case:** Full compaction cycle (tier 2 → tier 3 consolidation).
  - **Input:** Drive conversation through 4+ tier 1 compactions until tier 2 reaches 12% and full compaction triggers.
  - **Expected Outcome:** Tier 2 consolidates 4 oldest chunks into tier 3 recent archival, keeps 2 newest + new chunk = back to 6%. Historical header updated.

- **Test Case:** Topic pivot handling.
  - **Input:** Topic pivots every 20% of budget create semantic islands.
  - **Expected Outcome:** Compaction preserves key content from each island. Boundaries between islands don't produce garbled summaries.

- **Test Case:** Token compression ratio sweep (when LLMLingua-2 enabled).
  - **Input:** Run progressive test at compression ratios: off, 0.7, 0.5, 0.3.
  - **Expected Outcome:** Quality ratings compared across ratios. Compression floor identified where quality degrades.

**Scripts:** `scripts/stress-test-primer/primer.py` (priming), `scripts/stress-test-primer/runner.py` (orchestration).

**Investigation:** All findings follow the debug loop from `debugging-and-troubleshooting-guide.md`. 3-CLI root cause analysis on every non-trivial finding. Do not proceed to next budget tier with unresolved findings.
...
- **Maximum Throughput Benchmarking:** This plan does not include large-scale load testing to determine maximum requests-per-second or latency at scale. Context stress testing (compaction under realistic token volumes) is covered in §4.30.
...
| T-30.1 | Stuck job watchdog embedding timeout | 4.29 | Unit | | Not started | | Not run | Configurable timeout |
| T-30.2 | Stuck job watchdog extraction timeout | 4.29 | Unit | | Not started | | Not run | Configurable timeout |
| T-31.1 | Progressive compaction 8K→1M | 4.30 | Stress | runner.py | Not started | | Not run | Full progressive run |
| T-31.2 | Cold recall after compaction | 4.30 | Stress | runner.py | Not started | | Not run | Per-tier recall probes |
| T-31.3 | Full compaction cycle (tier 2→3) | 4.30 | Stress | runner.py | Not started | | Not run | 4+ tier 1 compactions |
| T-31.4 | Topic pivot handling | 4.30 | Stress | runner.py | Not started | | Not run | Semantic island preservation |
| T-31.5 | Token compression ratio sweep | 4.30 | Stress | runner.py | Not started | | Not run | Off/0.7/0.5/0.3 ratios |