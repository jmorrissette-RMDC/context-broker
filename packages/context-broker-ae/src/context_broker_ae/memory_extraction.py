...
def clean_for_compaction(text: str) -> str:
    ...
    Unlike _clean_for_extraction which replaces paths with [path], this function
    keeps file paths, function names, and entity names in the text so they survive
    as searchable hooks in the tier 2/3 summaries. Code blocks and formatting noise
    are still stripped — the LLM summarizes the discussion about artifacts, not the
