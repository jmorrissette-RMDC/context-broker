...
# Fork: quality gate and expiration imports removed — moved to CEA quality wrapper.
# Retained: _skip_graph parameter on add() for per-call graph control.
...
class Memory(MemoryBase):
    ...
    def add(
        ...
        *,
        ...
        _skip_graph: bool = False,  # FORK CB-24: skip graph operations for this call only
    ):
        """
        Create a new memory.
        ...
        # Fork: expiration cleanup removed — handled by CEA quality wrapper.
        ...
        # FORK CB-24: per-call graph disable. Extraction callers pass
        # _skip_graph=True to avoid the 30-70s graph bottleneck without
        # affecting the global enable_graph state.
        _use_graph = self.enable_graph and not _skip_graph

        with concurrent.futures.ThreadPoolExecutor() as executor:
            future1 = executor.submit(self._add_to_vector_store, messages, processed_metadata, effective_filters, infer)
            future2 = executor.submit(self._add_to_graph, messages, effective_filters) if _use_graph else None

            futures = [future1] + ([future2] if future2 else [])
            concurrent.futures.wait(futures)

            vector_store_result = future1.result()
            graph_result = future2.result() if future2 else None

        if _use_graph and graph_result is not None:
            return {
    ...
    def _add_to_vector_store(self, messages, metadata, filters, infer):
        ...
        # --- FORK: Side-channel metadata + quality gate ---
        # We extract metadata from structured facts BEFORE normalize_facts
        ...
        try:
            ...
            if not response.strip():
                ...
                try:
                    raw_facts = json.loads(response, strict=False)["facts"]
                except json.JSONDecodeError:
                    extracted_json = extract_json(response)
                    raw_facts = json.loads(extracted_json, strict=False)["facts"]
        ...
        # Fork: quality gate checkpoint 2 removed — handled by CEA quality wrapper.
        actions_list = new_memories_with_actions.get("memory", [])
    ...
    def get(self, memory_id):
        ...
        # Fork: expiration filtering removed — handled by CEA quality wrapper.
    ...
    def update(self, memory_id, data, metadata: Optional[Dict[str, Any]] = None):
        ...
        Example:
            >>> m.update(memory_id="mem_123", data="Likes tennis")
    ...
    def delete(self, memory_id):
        ...
        # 1.0.8: Clean up graph entities before deleting from vector store
        if self.enable_graph:
            try:
    ...
    def _update_memory(self, memory_id, data, existing_embeddings, metadata=None):
        ...
        # --- FORK: Preserve ALL existing payload keys, not just session IDs ---
        # Stock Mem0 only preserves user_id, agent_id, run_id, actor_id, role.
        # This drops any custom metadata (expires_at, x_category, etc.) on UPDATE.
        # The fork preserves everything from the existing payload, then merges
        # new metadata over it. Missing expires_at in new = keep existing.
        # Explicit null = clear (become permanent).
...
# NOTE: Fork changes (quality gate, metadata side-channel, expiration) have been
# removed from the sync Memory class — they now live in the CEA quality wrapper.
# The only remaining fork modification is _skip_graph on add() and elementId
# exposure in graph_memory.py search(). AsyncMemory does not need these changes
# because the Context Broker uses sync Memory via run_in_executor exclusively.
# If the async path is ever needed, the _skip_graph parameter would need to be
# added to AsyncMemory.add() as well.
